...
from utils_my.timer import Timer
from utils_my.pascal_voc import pascal_voc
...
class Solver(object):
    ...
    def __init__(self, net, data):   #Yolon_net and pascal_voc_data
        ...
        self.variable_to_restore = tf.compat.v1.global_variables(
        )  #初始化tensorflow的全局变量
        self.saver = tf.compat.v1.train.Saver(self.variable_to_restore,
                                              max_to_keep=None)  #定义tf.saver
        self.ckpt_file = os.path.join(self.output_dir, 'yolo.ckpt')  #定义保存模型输出的权重文件
        self.summary_op = tf.compat.v1.summary.merge_all(
        )  #将tensorflow各个操作联合起来，省事
        self.writer = tf.compat.v1.summary.FileWriter(
            self.output_dir, flush_secs=60)  #将内容写入到文件中，每60秒更新一次

        self.global_step = tf.compat.v1.train.create_global_step()  #创建全局的步骤
        self.learning_rate = tf.compat.v1.train.exponential_decay(  #设定变化的学习率，这个可以yolo论文中的相关指导来设定
            ...
            self.staircase,
            name='learning_rate')
        self.optimizer = tf.compat.v1.train.GradientDescentOptimizer(  #采用的优化方法是随机梯度下降
        ...
        self.sess.run(tf.compat.v1.global_variables_initializer())  #初始化全局变量
